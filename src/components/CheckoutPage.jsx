...
const CheckoutPage = () => {
  ...
  const updateQuantity = (id, increment) => {
    setServices(services.map(service => 
      service.id === id 
        ? { ...service, quantity: Math.max(0, service.quantity + (increment ? 1 : -1)) }
        : service
    ));
  };
  ...
  const finalTotal = serviceTotal + tipAmount + couponDiscount + walletUsed + sumoCharges + cancellationCharges + safetyCharges;
        ...
        <div className="w-1/2 space-y-6">
          ...
          <div className="bg-white rounded-lg p-6 shadow-sm border">
            {services.map((service, index) => (
              <div key={service.id} className={`flex items-center justify-between ${index > 0 ? 'mt-4 pt-4 border-t' : ''}`}>
                ...
                <div className="text-right">
                  <p className="font-semibold text-gray-900">₹{service.price * service.quantity}</p>
                </div>
              </div>
            ))}
            ...
            <div className="mt-6 pt-4 border-t">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="border rounded-lg p-4">
                  ...
                  <p className="text-sm font-medium">Complete wax (All in One)</p>
